...
class FacesDatabase:
        ...
        for num, path in enumerate(paths):
            ...
            for i, roi in enumerate(rois):
                ...
                if face_detector:
                    ...
                    if mm < 0:
                        crop = orig_image[
                            int(roi.position[1]) : int(roi.position[1] + roi.size[1]),
    ...
    def ask_to_save(self, image):
        ...
        img = cv2.copyMakeBorder(
            resized, 5, 5, 5, 5, cv2.BORDER_CONSTANT, value=(255, 255, 255)
        )
        cv2.putText(
            img,
    ...
    def dump_faces(self, image, desc, name):
        ...
        if match < 0:
            ...
            filename = "{}-{}.jpg".format(
                label, len(self.database[match].descriptors) - 1
            )
        filename = osp.join(self.fg_path, filename)

        log.debug(
            "Dumping image with label {} and path {} on disk.".format(label, filename)
        )
        if osp.exists(filename):
            log.warning(
                "File with the same name already exists at {}. So it won't be stored.".format(
                    self.fg_path
                )
            )
    ...
    def add_item(self, desc, label):
        ...
        if not label:
            ...
            log.warning(
                "Trying to store an item without a label. Assigned label {}.".format(
                    label
                )
            )
        else:
    ...
    def savePickle(self, fileName):
        if len(self.database) > 0:
            with open(fileName, "wb") as f:
                ...
                f.close()
                log.info("[INFO] face database {} saved".format(fileName))

    def loadPickle(self, fileName):
        self.database = pickle.loads(open(fileName, "rb").read())
        log.info("[INFO] face database loaded from {}.".format(fileName))
