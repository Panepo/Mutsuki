...
def build_argparser():
    ...
    gallery.add_argument(
        ...
        default="LOAD",
        type=str,
        help="Command of face database operation, LOAD, SAVE and NONE",
    )
    gallery.add_argument(
        "-fg",
        metavar="PATH",
    ...
    gallery.add_argument(
        "-fps",
        metavar="PATH",
        default="./database/faces.pickle",
        help="Path to the face images pickle file save to.",
    )
    gallery.add_argument(
        "-fpl",
        metavar="PATH",
        default="./database/faces.pickle",
        help="Path to the face images pickle file loaded from.",
    )
    gallery.add_argument(
        "--run_detector",
        action="store_true",
    ...
    models.add_argument(
        "-m_fd",
        metavar="PATH",
        default="./models/face-detection-retail-0004.xml",
        help="Path to the Face Detection model XML file",
    )
    models.add_argument(
        "-m_lm",
        metavar="PATH",
        default="./models/landmarks-regression-retail-0009.xml",
        help="Path to the Facial Landmarks Regression model XML file",
    )
    models.add_argument(
        "-m_reid",
        metavar="PATH",
        default="./models/face-reidentification-retail-0095.xml",
...
class FrameProcessor:
    ...
    def __init__(self, args):
        ...
        self.face_identifier = FaceIdentifier(face_reid_net, match_threshold=args.t_id)

        self.face_detector.deploy(args.d_fd, context)
        self.landmarks_detector.deploy(args.d_lm, context, queue_size=self.QUEUE_SIZE)
        self.face_identifier.deploy(args.d_reid, context, queue_size=self.QUEUE_SIZE)
        ...
        log.info(
            "Database is built, registered %s identities" % (len(self.faces_database))
        )
        ...
        if self.allow_grow and len(unknowns) > 0:
            for i in unknowns:
                ...
                if (
                    ...
                    continue
                crop = orig_image[
                    int(rois[i].position[1]) : int(
                        rois[i].position[1] + rois[i].size[1]
                    ),
                    int(rois[i].position[0]) : int(
