...
class FaceIdentifier(Module):
    ...
    def __init__(self, model, match_threshold=0.5):
        ...
        assert (
            len(model.outputs[self.output_blob].shape) == 4
            or len(model.outputs[self.output_blob].shape) == 2
        ), (
    ...
    def get_transform(src, dst):
        assert (
            np.array_equal(src.shape, dst.shape) and len(src.shape) == 2
    ...
    def _align_rois(self, face_images, face_landmarks):
        ...
        for image, image_landmarks in zip(face_images, face_landmarks):
            ...
            desired_landmarks = (
                np.array(self.REFERENCE_LANDMARKS, dtype=np.float64) * scale
            )
            ...
            cv2.warpAffine(
                img, transform, tuple(scale), img, flags=cv2.WARP_INVERSE_MAP
            )
