...
def build_argparser():
    ...
    args.add_argument(
        ...
        default="./models/instance-segmentation-security-0050.xml",
        type=str,
        metavar='"<path>"',
    )
    args.add_argument(
    ...
    args.add_argument(
        ...
        default="/opt/intel/openvino_2019.3.376/inference_engine/lib/intel64/libcpu_extension_avx2.so",
        type=str,
    ...
    args.add_argument(
        "--no_track", help="Optional. Disable tracking.", action="store_true"
    )
    args.add_argument(
        "--show_scores", help="Optional. Show detection scores.", action="store_true"
    )
    args.add_argument(
        "--show_boxes", help="Optional. Show bounding boxes.", action="store_true"
    )
    args.add_argument(
        "-pc",
        "--perf_counts",
...
def main():
    ...
    assert required_input_keys == set(
        net.inputs.keys()
    ), "Demo supports only topologies with the following input keys: {}".format(
        ", ".join(required_input_keys)
    )
    required_output_keys = {"boxes", "scores", "classes", "raw_masks"}
    assert required_output_keys.issubset(
        net.outputs.keys()
    ...
    visualizer = Visualizer(
        class_labels, show_boxes=args.show_boxes, show_scores=args.show_scores
    )
    ...
    while cap.isOpened():
        ...
        input_image = np.pad(
            input_image,
            ((0, h - input_image_size[0]), (0, w - input_image_size[1]), (0, 0)),
            mode="constant",
            constant_values=0,
        )
        ...
        if len(boxes) and args.raw_output_message:
            ...
            for box, cls, score, mask in zip(boxes, classes, scores, masks):
                log.info(
                    "{:>10} | {:>10f} | {:>8.2f} | {:>8.2f} | {:>8.2f} | {:>8.2f} ".format(
                        cls, score, *box
                    )
                )
