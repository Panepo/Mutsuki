...
class ResultRenderer(object):
    def __init__(
        ...
        self.postprocessing = [
            LabelPostprocessing(n_frames=30, history_size=100)
    ...
    def render_frame(self, frame, logits, timers, frame_ind):
        ...
        if logits is not None:
            labels, probs = decode_output(
                logits,
                self.lables,
                top_k=self.number_of_predictions,
                label_postprocessing=self.postprocessing,
            )
            print(
                "Frame {}: {} - {:.2f}% -- {:.2f}ms".format(
                    frame_ind, labels[0], probs[0] * 100, inference_time
                )
            )
        else:
            labels = ["Preparing..."]
        ...
        if self.display_fps:
            ...
            cv2.putText(
                frame,
                "Inference time: {:.2f}ms ({:.2f} FPS)".format(inference_time, fps),
                text_loc,
                FONT_STYLE,
                FONT_SIZE,
                FONT_COLOR,
...
def fill_area(image, bottom_left, top_right, color=(0, 0, 0), alpha=1.0):
    """Fills area with the specified color"""
    xmin, ymax = bottom_left
    xmax, ymin = top_right

    image[ymin:ymax, xmin:xmax, :] = (
        image[ymin:ymax, xmin:xmax, :] * (1 - alpha) + np.asarray(color) * alpha
    )
    return image
